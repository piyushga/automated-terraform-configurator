...
export default function AwsResultPage() {
    ...
    // if user navigates directly to this URL without state, show message
        ...
        return (
            <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6">
                <div className="bg-white rounded-xl shadow p-6 max-w-xl w-full text-center">
                    ...
                    <p className="text-sm text-gray-600 mb-4">This page expects result data from the create flow.</p>
                    <div className="flex justify-center">
                        <button onClick={() => nav("/")} className="px-4 py-2 rounded bg-blue-600 text-white">Go Home</button>
                    </div>
                </div>
            </div>
        );
    ...
    const ok = state.ok !== false; // default true
            ...
            <div className="max-w-3xl mx-auto bg-white rounded-xl shadow p-6">
                ...
                {ok ? (
                    ...
                    <div className="rounded border p-4 bg-red-50 mb-4">
                        ...
                        <pre className="text-sm text-gray-800 whitespace-pre-wrap mt-2">{state.error ?? JSON.stringify(state.raw ?? state)}</pre>
                    </div>
                )}

                <div className="flex gap-3 justify-end">
                    <button onClick={() => nav("/")} className="px-4 py-2 rounded bg-emerald-600 text-white">Create Another Resource</button>
                    <button onClick={() => nav("/")} className="px-4 py-2 rounded border">Close</button>
                </div>
            </div>
        </div>
    );
